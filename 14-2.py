...
def generate(recipes:Dict[str,Recipe], cur:str, amount:int, made:DefaultDict[str,int], extras:DefaultDict[str,int], depth:int) -> Tuple[int, int]:
    ...
    if amount % curRecipe.output.amount:
        #print('BAD', amount, curRecipe)
        exit()
    multiplier:int = amount // curRecipe.output.amount

    for inputChem in curRecipe.inputs:
        inputRecipe:Chemical = recipes[inputChem.name]
        #print(' ' * depth, 'get_min', inputChem, inputRecipe, amount, multiplier)
        next_amts:int = get_min_amount(inputChem, inputRecipe, multiplier)
...
ore_used:Dict[int,int] = dict()
fuel = 2
prev_fuel_h:int = None
prev_fuel_l:int = None
mode = 0
while True:
    with open('14.txt') as f:
        ...
        recipes['FUEL'].output.amount = fuel
        for i in recipes['FUEL'].inputs:
            i.amount *= recipes['FUEL'].output.amount
        ...
        generate(recipes, 'FUEL', recipes['FUEL'].output.amount, made, extras, 0)
        ...
        while numReduced > 0:
            ...
            for chemName, amtMade in extras.items():
                ...
                if recipe.inputs[0].name == 'ORE':
                    ...
                    mult = reduced // recipe.output.amount
                    for i in recipe.inputs:
                        #print('  adding reduced', i, mult, i.amount * mult)
                        extras[i.name] += i.amount * mult

        ore_used[fuel] = made['ORE']

        if mode == 0 and made['ORE'] > 1000000000000:
            new_fuel = (fuel - prev_fuel_h) // 2 + prev_fuel_h
            prev_fuel_l = prev_fuel_h
            prev_fuel_h = fuel
            fuel = new_fuel
            mode = 1
            print('binary ore=', made['ORE'], 'f=', prev_fuel_l, fuel, prev_fuel_h)
            continue

        if mode == 0:
            print('check, ore=', made['ORE'], 'f=', fuel)

        if mode == 0:
            prev_fuel_h = fuel
            fuel *= 2
        elif mode == 1:
            if made['ORE'] > 1000000000000:
                new_fuel = (fuel - prev_fuel_l) // 2 + prev_fuel_l
                prev_fuel_h = fuel
                fuel = new_fuel
                print('too many ore=', made['ORE'], 'f=', prev_fuel_l, fuel, prev_fuel_h)
            else:
                new_fuel = (prev_fuel_h - fuel) // 2 + fuel
                prev_fuel_l = fuel
                fuel = new_fuel
                print('too few ore=', made['ORE'], 'f=', prev_fuel_l, fuel, prev_fuel_h)

            if prev_fuel_l == fuel or prev_fuel_h == fuel:
                print('done', made['ORE'], fuel)
                #print(ore_used)
                exit()
